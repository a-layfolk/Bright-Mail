...
public:
    ...
    bool Add_Recived_Email(const char *targetTelephone, const char *emailTitle, const char *emailContent, const char *attachedFile)
    {
        ...
        {
            sprintf(query, "Insert into Email(myId,targetTelephone,emailType,emailTitle,emailContent,emailTime,attachedFile)values(%s,%s,'%s','%s','%s','%s');", myId, targetTelephone, "received_unread", emailTitle, emailContent, time, attachedFile);
        }
...
{
    Sql_Core SQL(CONFIG::sql_ip, CONFIG::sql_user, CONFIG::sql_password, CONFIG::sql_db, CONFIG::sql_port);
    SQL.Connect();
    char one[10];
    char two[10];
    char three[10];
    ...
    // cin >> one;
    // cin >> two;
    // char *id = SQL.Sign_In(one, two);

    // cin >> three;
    // char *id = SQL.Sign_Up(one, two, three);
    // cout << id;
    //-------------------------------------------------登陆
    ...
    // cin >> four;
    // cin >> five;
    // SQL.Add_Email(id, one, two, three, four);
    // SQL.Add_Email_File(id, "15815589508", "send", "FirstMial", "哈哈哈哈哈哈", "file_name");

    //----------------------------------------------------添加联系人
    // cout << "Add Contact:" << endl;
    // cin >> one;
    // cin >> two;
    // SQL.Add_Contact(id, one, two);

    //-----------------------------------------------------获取邮件列表
    // cout << SQL.Get_Email_List_JSON("1", "send");

    //-------------------------------------------------------获取联系人列表
    // cout << SQL.Get_Contact_List_JSON("1");
    // cout << SQL.Get_Email_Detail_JSON("2");
    // SQL.Add_Email("1", "1589", "send", "哇！", "这个邮件没有附件！");
    SQL.Add_Email("1", "133", "send", "测试测试", "这个是内容", NULL);
    SQL.Add_Recived_Email("133",  "测试测试", "这个是内容", NULL);
    SQL.Close();
    return 0;
}
