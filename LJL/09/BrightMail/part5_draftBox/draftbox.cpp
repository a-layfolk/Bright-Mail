    ...
    : QWidget(parent)
{
    ...
    //    CLIENT::EMAIL_INFO *newTableList = CC.Get_Mail_List("1","draft", &return_size);
    ...
    header_failure<<tr("邮件标题") << tr("收件人") << tr("发送时间") << tr("邮件编号");
    ...
    for(int i = 0; i < row_count; i++)
    {
        ...
        item0->setText(QString("草稿 %1").arg(i));
        item1->setText("张三");
        item2->setText("2020-9-16");
    ...
    connect(emailWidget, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(tableDoubleClicked_send_slot(QModelIndex)));
...
void DraftBox::tableDoubleClicked_send_slot(QModelIndex index)
{
    ...
    details_draft *new_deli_draft = new details_draft;

    new_deli_draft->sendData(emailId);
