...
/*---------------------------------连接数据库，如果成功--------------------------------*/
    ...
    CLIENT::EMAIL_INFO *newTableList = CC.Get_Mail_List("1","sended", &return_size);
...
//        item0->setText(QString("已发送 %1").arg(i));
//        item1->setText("张三");
//        item2->setText("2020-09-15");
...
void Deliver::tableDoubleClicked(QModelIndex index)
{
...
/*-----------------------------------------------这里是一段之前用过的代码，现在用不到，但我觉得不错---------------------------------------------------------*/
    ...
    deliver_details *new_deli_del = new deliver_details;

    new_deli_del->sendData(emailId);
